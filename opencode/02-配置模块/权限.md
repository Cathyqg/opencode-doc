...
> 官方页面：https://opencode.ai/docs/zh-cn/permissions
> 源码来源：https://raw.githubusercontent.com/sst/opencode/refs/heads/dev/packages/web/src/content/docs/zh-cn/permissions.mdx
> 页面说明：控制哪些操作需要审批才能运行。

## 理解

- 权限页定义的是动作审批策略，而不是工具是否存在；前者解决“能不能直接做”，后者解决“工具看不看得到”。
- 规则支持全局默认值以及按工具、按模式细分，匹配时以后出现的更具体规则覆盖更早规则。
- `external_directory` 和 `doom_loop` 是额外的安全护栏，分别防止越出工作区和陷入重复调用循环。
- 代理可以覆盖全局权限，所以常见做法是给只读评审代理更严格的 `edit` / `bash` 限制。
