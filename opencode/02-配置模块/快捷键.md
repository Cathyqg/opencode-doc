...
> 官方页面：https://opencode.ai/docs/zh-cn/keybinds
> 源码来源：https://raw.githubusercontent.com/sst/opencode/refs/heads/dev/packages/web/src/content/docs/zh-cn/keybinds.mdx
> 页面说明：自定义您的快捷键。

## 理解

- 快捷键主要服务 TUI，几乎所有交互动作都能在 `keybinds` 里重映射。
- `<leader>` 是整套快捷键设计的核心前导键，修改它会连带影响大量默认组合。
- 文档除了给配置项，还专门解释了禁用按键、桌面版输入区快捷键以及 `Shift+Enter` 的终端兼容问题。
- 真正修改前最好先保留少量变更，因为整套全改会直接影响肌肉记忆和后续排障成本。
