...
> 官方页面：https://opencode.ai/docs/zh-cn/themes
> 源码来源：https://raw.githubusercontent.com/sst/opencode/refs/heads/dev/packages/web/src/content/docs/zh-cn/themes.mdx
> 页面说明：选择内置主题或定义您自己的主题。

## 理解

- 主题页处理的是 TUI 视觉层，前提是终端本身具备足够的颜色能力。
- 可以直接选内置主题、使用 `system` 跟随终端配色，或在全局 / 项目目录下放自定义主题文件。
- 自定义主题支持 `defs` 复用颜色，并按层级覆盖，这适合团队在项目内固定一套视觉风格。
- 如果终端不支持某些颜色能力，主题效果会退回到终端默认值，所以显示结果与终端实现有关。
