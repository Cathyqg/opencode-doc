...
> 官方页面：https://opencode.ai/docs/zh-cn/mcp-servers
> 源码来源：https://raw.githubusercontent.com/sst/opencode/refs/heads/dev/packages/web/src/content/docs/zh-cn/mcp-servers.mdx
> 页面说明：添加本地和远程 MCP 工具。

## 理解

- MCP 页面讲的是如何把外部工具系统接进 OpenCode，包括本地进程型 MCP 和远程 HTTP MCP。
- 配置重点不只是地址和命令，还包括 OAuth、预注册、认证调试以及按代理启用范围。
- 管理章节说明 MCP 可以全局可用，也可以只对某些代理开放；这对隔离高风险工具尤其重要。
- 示例部分展示的是接入模式，真正部署时要重点检查认证方式、请求头与工具暴露边界。
