...
> 官方页面：https://opencode.ai/docs/zh-cn/rules
> 源码来源：https://raw.githubusercontent.com/sst/opencode/refs/heads/dev/packages/web/src/content/docs/zh-cn/rules.mdx
> 页面说明：为 opencode 设置自定义指令。

## 理解

- 规则页讲的是“给代理长期指令”，核心载体是 `AGENTS.md` 与配置中的 `instructions`。
- 规则有项目级、全局级以及 Claude / Agents 兼容路径；项目内规则通常更贴近仓库现实，适合承载团队规范。
- 文档强调了优先级与合并顺序，这决定同一主题出现冲突时到底哪条规则生效。
- 外部文件引用让长规范可以拆分管理，而不必把所有约束都塞进一个 `AGENTS.md`。
