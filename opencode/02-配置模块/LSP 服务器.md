...
> 官方页面：https://opencode.ai/docs/zh-cn/lsp
> 源码来源：https://raw.githubusercontent.com/sst/opencode/refs/heads/dev/packages/web/src/content/docs/zh-cn/lsp.mdx
> 页面说明：OpenCode 与你的 LSP 服务器集成。

## 理解

- LSP 集成让 OpenCode 能借现有语言服务器做符号、定义和类型相关查询，不必完全依赖文本搜索。
- 文档区分内置支持、工作原理和自定义服务器配置；默认能用的语言有限，复杂场景通常需要补充自定义 LSP。
- 环境变量和初始化选项用于把 OpenCode 接到现有语言工具链上，尤其适合 monorepo 或需要额外启动参数的场景。
- 如果某个语言服务器不稳定，可以按语言禁用或改成自定义配置，而不是全局关掉 LSP。
