...
> 官方页面：https://opencode.ai/docs/zh-cn/custom-tools
> 源码来源：https://raw.githubusercontent.com/sst/opencode/refs/heads/dev/packages/web/src/content/docs/zh-cn/custom-tools.mdx
> 页面说明：创建 LLM 可在 opencode 中调用的工具。

## 理解

- 自定义工具是把本地脚本包装成 LLM 可调用的结构化工具，比单纯让模型拼 bash 命令更稳定。
- 目录结构、参数 schema 和上下文对象是这页的核心；工具描述越清晰，模型调用越可靠。
- 单文件多工具以及与内置工具重名时的冲突规则，决定了你能否把多个操作打包，以及如何避免覆盖问题。
- 这页和 `tools` 页是配套关系：前者定义工具本身，后者决定这些工具是否对代理可见。
