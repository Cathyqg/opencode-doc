...
> 官方页面：https://opencode.ai/docs/zh-cn/tools
> 源码来源：https://raw.githubusercontent.com/sst/opencode/refs/heads/dev/packages/web/src/content/docs/zh-cn/tools.mdx
> 页面说明：管理 LLM 可以使用的工具。

## 理解

- `tools` 解决的是“模型在会话里能看到哪些能力”，它比权限系统更靠前，是一层工具可见性控制。
- 内置工具覆盖文件读写、搜索、补丁、bash、LSP、web、提问等常见操作；其中 `write`、`patch` 等写操作最终都受 `edit` 权限约束。
- 自定义工具和 MCP 服务器都会并入同一工具体系，所以这一页和“自定义工具”“MCP 服务器”两页是配套关系。
- 忽略模式决定哪些文件和目录不会暴露给工具，是减少噪音和保护敏感路径的重要手段。
