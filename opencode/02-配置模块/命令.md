...
> 官方页面：https://opencode.ai/docs/zh-cn/commands
> 源码来源：https://raw.githubusercontent.com/sst/opencode/refs/heads/dev/packages/web/src/content/docs/zh-cn/commands.mdx
> 页面说明：为重复任务创建自定义命令。

## 理解

- 自定义命令的本质是把一段常用提示词包装成 `/命令`，既能用 JSON 配，也能用 `commands/*.md` 文件配。
- Markdown 命令通常更适合维护，因为 frontmatter 写元数据，正文写提示词，阅读和复用都更直接。
- 模板支持参数、shell 输出注入和文件引用，所以命令不是静态短语，而是可以携带上下文的工作流入口。
- `agent`、`subtask` 和 `model` 选项决定这条命令由谁执行、是否拆成子任务以及具体使用哪个模型。
