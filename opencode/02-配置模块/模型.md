...
> 官方页面：https://opencode.ai/docs/zh-cn/models
> 源码来源：https://raw.githubusercontent.com/sst/opencode/refs/heads/dev/packages/web/src/content/docs/zh-cn/models.mdx
> 页面说明：配置 LLM 提供商和模型。

## 理解

- 模型配置分成两层：先接入提供商，再在 `model` 中选择默认模型、轻量模型以及需要暴露出来的模型别名或变体。
- 推荐模型与默认模型是两个概念：前者提供选型建议，后者决定实际会话的默认值。
- 变体机制适合把同一基础模型包装成不同用途，例如高推理版本、低成本版本或带特定参数的版本。
- 团队环境里最好显式写出模型 ID 和自定义变体，而不是完全依赖自动发现，这样行为更稳定。
