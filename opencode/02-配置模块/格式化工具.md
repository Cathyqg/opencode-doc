...
> 官方页面：https://opencode.ai/docs/zh-cn/formatters
> 源码来源：https://raw.githubusercontent.com/sst/opencode/refs/heads/dev/packages/web/src/content/docs/zh-cn/formatters.mdx
> 页面说明：OpenCode 使用特定语言的格式化工具。

## 理解

- 格式化工具是文件修改后的自动收尾层，按语言调用合适的 formatter，而不是让模型自己尽量保持格式。
- 文档区分了内置 formatter、工作原理、禁用方式和自定义 formatter，这意味着团队既能统一风格，也能为个别语言做例外。
- 如果某门语言的默认 formatter 不合适，优先替换成自定义命令，而不是整体关闭格式化。
- 这一页和编辑工具、权限页联动很强，因为 formatter 往往在写文件后自动触发。
