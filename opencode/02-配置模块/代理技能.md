...
> 官方页面：https://opencode.ai/docs/zh-cn/skills
> 源码来源：https://raw.githubusercontent.com/sst/opencode/refs/heads/dev/packages/web/src/content/docs/zh-cn/skills.mdx
> 页面说明：通过 SKILL.md 定义可复用的行为

## 理解

- 代理技能是可按需加载的复用能力包，载体是 `SKILL.md`，不是普通提示词片段。
- 发现机制同时兼容 `.opencode`、`.claude` 与 `.agents` 目录，这使 OpenCode 能复用已有生态里的技能定义。
- frontmatter 里的 `name` 和 `description` 是关键索引字段，命名、长度与目录名都有严格约束。
- 技能本身也受权限控制；如果某类技能不该被某个代理看到，可以在 `permission.skill` 或代理配置里覆盖。
