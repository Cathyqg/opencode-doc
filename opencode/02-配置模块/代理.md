...
> 官方页面：https://opencode.ai/docs/zh-cn/agents
> 源码来源：https://raw.githubusercontent.com/sst/opencode/refs/heads/dev/packages/web/src/content/docs/zh-cn/agents.mdx
> 页面说明：配置和使用专门的代理。

## 理解

- 代理页描述的是如何把一个会话拆成多个角色，区分主代理与子代理，并提供 `build`、`plan`、`general`、`explore`、`compaction`、`title`、`summary` 等内置代理。
- 代理既可以写在 `opencode.json`，也可以用 `agents/*.md` 的 frontmatter 定义；Markdown 方式更适合承载长提示词和角色说明。
- 每个代理都能独立配置模型、工具、权限、模式、隐藏状态、温度、`top_p`、最大步数等，相当于把“角色 + 执行边界”一起封装。
- 真正有价值的用法不是简单多建几个名字，而是让不同代理在职责和权限上明显分工，比如只读评审、文档维护或安全审计。
