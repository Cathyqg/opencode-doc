...
> 官方页面：https://opencode.ai/docs/zh-cn/acp
> 源码来源：https://raw.githubusercontent.com/sst/opencode/refs/heads/dev/packages/web/src/content/docs/zh-cn/acp.mdx
> 页面说明：在任何兼容 ACP 的编辑器中使用 OpenCode。

## 理解

- ACP 支持页不是在讲 OpenCode 自己的 UI，而是在讲如何把 OpenCode 作为 ACP 后端接入其他编辑器。
- 配置核心是让编辑器知道 OpenCode 的可执行入口或 ACP 配置文件，随后由编辑器侧插件发起调用。
- 文档列出的 Zed、JetBrains、Avante.nvim、CodeCompanion.nvim 代表不同生态的接入范式，而不是不同能力等级。
- 如果某个编辑器已经兼容 ACP，这页基本就是接入手册；实际问题通常出在编辑器侧的配置路径和命令参数。
