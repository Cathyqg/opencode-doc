...
> 官方页面：https://opencode.ai/docs/zh-cn/config
> 源码来源：https://raw.githubusercontent.com/sst/opencode/refs/heads/dev/packages/web/src/content/docs/zh-cn/config.mdx
> 页面说明：使用 OpenCode JSON 配置。

## 理解

- `opencode.json` 或 `opencode.jsonc` 是总入口，既能集中写配置，也能和 `.opencode/` 目录下的 `agents`、`commands`、`themes` 等子目录配合使用。
- 配置有明确优先级：远程 `.well-known/opencode`、全局目录、`OPENCODE_CONFIG` / `OPENCODE_CONFIG_CONTENT`、项目级配置逐层叠加，离当前项目越近的配置覆盖越远的配置。
- 这一页本质上是总 schema 说明，覆盖 `tui`、`server`、`tools`、`models`、`themes`、`agent`、`permission`、`mcp`、`formatter` 等几乎所有子模块。
- 变量替换支持 `{env:VAR}` 与 `{file:path}`，适合把密钥、证书或长文本从配置外部注入进来。
- `disabled_providers` 的优先级高于 `enabled_providers`，而 `experimental` 明确属于试验区，团队环境里应谨慎开启。
