...
> 官方页面：https://opencode.ai/docs/zh-cn/providers
> 源码来源：https://raw.githubusercontent.com/sst/opencode/refs/heads/dev/packages/web/src/content/docs/zh-cn/providers.mdx
> 页面说明：在 OpenCode 中使用任意 LLM 提供商。

## 理解

- 这页本质上是“各家 LLM 提供商接入手册”：每个 provider 都说明凭据、Base URL、额外请求头以及该厂商特有的配置项。
- 大多数接入都遵循同一套路：先准备 API Key 或云凭据，再在 `provider` 中声明提供商，最后在 `model` 中选择实际模型。
- 文档区分“内置提供商目录”和“自定义提供商”；前者开箱即用，后者用于兼容 OpenAI / Anthropic 风格但尚未内置的平台。
- 对接失败时优先排查凭据存储位置、Base URL、请求头，以及云厂商特有的区域、项目或租户配置。
